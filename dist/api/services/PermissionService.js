...
module.exports = {
  ...
  findModelPermissions: function findModelPermissions(options) {
    ...
    return User.findOne({ where: { id: options.user.id }, include: ['roles'] }).then(function (user) {
      ...
      return Permission.find({ permissionCriteria: permissionCriteria, include: ['criteria'] });
    });
  },
